...
const CartItem = ({ item, setRun = (f) => f, run = undefined }) => {
  ...
  const handleChange = (Item) => (event) => {
    if (event.target.value < 1) {
      ...
      return;
    } else setValues({ ...values, quantity: event.target.value });
                      ...
                      <Fragment>
                        <tr>
                          ...
                          <td className="align-middle">
                            <strong>{`$${foundItem.price} per ${foundItem.variant}`}</strong>
                          </td>

                          <td className="align-middle">
                            <button
                              type="button"
                              className="fa fa-minus btn btn-number"
                            ...
                            <input
                              ...
                              value={quantity}
                              onFocus={handleFocus}
                              onChange={handleChange(foundItem)}
                              style={{ width: "3rem" }}
                              min="1"
                            />
