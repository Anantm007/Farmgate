...
router.post("/payment/", async (req, res) => {
  try {
    ...
    ); // rapidEndpoint can be written as "Sandbox/Production"
    const {
      EWAY_CARDNAME,
      EWAY_CARDNUMBER,
      EWAY_CARDCVN,
    ...
    client
      ...
      .then(function (response) {
        ...
        if (response.get("TransactionStatus")) {
          console.log(
            "Payment successful! ID: " + response.get("TransactionID")
          );
          return res.json({
            success: true,
          ...
          errorCodes.forEach(function (errorCode) {
            console.log(
              "Response Message: " + rapid.getMessage(errorCode, "en")
            );
          });
          return res.json({
            success: false,
